/**
 * POST /api/banks/gocardless/link
 *   { connectionId, links: [{ externalId, finAccountId, iban?, currency }] }
 *
 * Creates BankAccountLink rows for the chosen accounts, marks the connection
 * ACTIVE, then runs an initial sync inline.
 */
...
export const POST = withAuth(async (userId, req) => {
  ...
  const conn = await prisma.bankConnection.findFirst({
    where: { id: data.connectionId, userId, provider: "gocardless" },
  });
  if (!conn) return NextResponse.json({ error: "Not found" }, { status: 404 });

  // Validate every FinAccount is owned by the user before any write.
  ...
  await prisma.$transaction([
    ...data.links.map((l) =>
      prisma.bankAccountLink.upsert({
        ...
        create: { userId, connectionId: data.connectionId, finAccountId: l.finAccountId, externalId: l.externalId, iban: l.iban, currency: l.currency },
  ...
  await recordAudit({
    userId, action: "bank.gocardless.link", targetType: "BankConnection", targetId: data.connectionId,
